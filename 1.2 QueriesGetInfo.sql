...
-- Query 26
SELECT CONVERT(decimal(18,2), user_seeks * avg_total_user_cost * (avg_user_impact )) AS [index_advantage], 
...
-- Query 30 ----puede demorar un poco si la instancia esta ocupada 
...
-- Query 34 -- ejecutar varias veces durante aprox 5 minutos cada 10 segundos
...
-- Query 36
SELECT TOP(50) DB_NAME(t.[dbid]) AS [Database Name], OBJECT_NAME(t.objectid, t.dbid) AS object_name,--LEFT(t.[text], 50) AS [Short Query Text],  
...
union all
SELECT @@SERVERNAME AS [Server Name],[object_name],[counter_name],[cntr_value] 
FROM sys.dm_os_performance_counters WITH (NOLOCK)
...
cp.cacheobjtype, cp.objtype, cp.size_in_bytes/1024 AS [Plan Size in KB]
